...
const Menu = () => {
  ...
  useEffect(() => {
    ...
    const hasInvalidItems = cart.some(
      (item) => !menuItems.find((m) => m.id.toString() === item.idMeal),
    );
    if (hasInvalidItems) {
      dispatch(clearCart());
    }
  ...
  const totalPrice = cart.reduce(
    (acc, item) =>
      acc +
      item.quantity *
        (menuItems.find((m) => m.id.toString() === item.idMeal)?.price || 10),
    0,
  );
  ...
  const handleOrderSubmit = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      alert(
        `მადლობა! შეკვეთა მიღებულია. მეთოდი: ${paymentMethod === "card" ? "ბარათი" : "ნაღდი"}`,
      );
      dispatch(clearCart());
      setIsCheckout(false);
    },
  ...
  return (
    <Section
      title={isCheckout ? "შეკვეთის დასრულება" : "ჩვენი მენიუ"}
      subtitle={
        isCheckout
          ? "შეავსეთ მონაცემები"
          : "ყავა, დესერტები და სასმელები ქუთაისში"
      }
    >
                  ...
                  >
                    ...
                    <div className="p-6 flex flex-col flex-grow justify-between">
                      ...
                      <div className="mt-4 flex items-center justify-between">
                        <span className="text-orange-600 font-black text-lg md:text-xl">
                        ...
                        {inCart ? (
                          <div className="flex items-center bg-gray-900 rounded-xl p-1 text-white">
                            <button
                              ...
                              className="w-8 h-8 hover:bg-gray-700 rounded-lg transition-colors"
              ...
              <form onSubmit={handleOrderSubmit} className="space-y-6">
                ...
                {paymentMethod === "card" && (
                  <div className="p-6 bg-gray-900 rounded-3xl text-white space-y-4 shadow-lg animate-fadeIn">
                    <input
                      required
                      aria-label="ბარათის ნომერი"
                      placeholder="ბარათის ნომერი"
                      className="bg-transparent border-b border-gray-700 w-full p-2 outline-none focus:border-orange-500 transition-colors"
                    />
                    <div className="flex gap-4">
                      <input
                        required
                        aria-label="ვადის გასვლის თვე და წელი"
                        placeholder="MM/YY"
                        className="bg-transparent border-b border-gray-700 w-full p-2 outline-none focus:border-orange-500 transition-colors"
                      />
                      <input
                        required
                        aria-label="ბარათის უსაფრთხოების კოდი"
                        placeholder="CVC"
                        className="bg-transparent border-b border-gray-700 w-full p-2 outline-none focus:border-orange-500 transition-colors"
                      />
                    </div>
                ...
                <button
                  type="submit"
                  className="w-full bg-orange-600 text-white py-5 rounded-2xl font-bold text-lg md:text-xl shadow-lg hover:bg-blue-700 active:scale-95 transition-all duration-300"
                >
                  შეკვეთის გაფორმება
                </button>
              </form>
        ...
        <aside className="lg:w-[380px] w-full lg:sticky lg:top-28">
          <div className="bg-white rounded-[2.5rem] p-6 md:p-8 shadow-2xl border border-gray-50">
            ...
            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {cart.length === 0 ? (
                <p className="text-gray-400 text-center py-10 text-sm italic">
                  კალათა ცარიელია
                </p>
              ) : (
                cart.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <img
                        ...
                        height="40"
                      />
                      <span className="text-xs font-bold truncate w-24 sm:w-32">
                        {item.strMeal}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
              ...
              <div className="mt-8 pt-6 border-t border-dashed border-gray-100">
                ...
                {!isCheckout && (
                  <button
                    ...
                    className="w-full bg-orange-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-blue-700 active:scale-95 transition-all duration-300 shadow-lg"
                  >
                    გაფორმება
                  </button>
                )}
