...
const cartSlice = createSlice({
  ...
  reducers: {
    addItem: (state, action: PayloadAction<Omit<CartItem, 'quantity'>>) => {
      const existing = state.items.find(i => i.idMeal === action.payload.idMeal);
      if (existing) {
        existing.quantity += 1;
      } else {
    ...
    removeItem: (state, action: PayloadAction<{ id: string; delta: number }>) => {
      const item = state.items.find(i => i.idMeal === action.payload.id);
      if (item) {
        item.quantity += action.payload.delta;
        if (item.quantity <= 0) {
          state.items = state.items.filter(i => i.idMeal !== action.payload.id);
        }
      }
      localStorage.setItem('cafe_moon_cart', JSON.stringify(state.items));
    },
